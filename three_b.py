...
def instruction_to_coords(current_coord, instruction):
    direction = instruction[0]
    magnitude = int(instruction[1:])

    step = STEPS_BY_DIRECTION[direction]

    new_coords = []
    last_coord = current_coord

    for _ in range(0, magnitude):
        last_coord = add_step(last_coord, step)
        new_coords.append(last_coord)

    return new_coords

def total_wire_steps_to_intersection(wire_1, wire_2, intersection):
    wire_1_dist = wire_1.index(intersection) + 1
    wire_2_dist = wire_2.index(intersection) + 1

    return wire_1_dist + wire_2_dist


def wire_to_coords(wire_instructions):
    origin = (0, 0)
    wire_as_coords = [origin]
    current_coord = origin

    for instruction in wire_instructions:
        new_coords = instruction_to_coords(current_coord, instruction)
        wire_as_coords.extend(new_coords)
        current_coord = wire_as_coords[-1]

    return wire_as_coords

wire_1 = wire_to_coords(wire_1_instructions)[1:]
wire_2 = wire_to_coords(wire_2_instructions)[1:]

intersections = list(set(wire_1) & set(wire_2))

intersection_distances = [ total_wire_steps_to_intersection(wire_1, wire_2, intersection) for intersection in intersections ]

lowest_distance = intersection_distances[0]

for distance in intersection_distances[1:]:
