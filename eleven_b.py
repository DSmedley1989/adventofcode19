...
class Canvas:
    ...
    def __str__(self):
        ...
        for _, panel in self.panels.items():
            if panel.x < lowest_x:
                lowest_x = panel.x
            if panel.x > highest_x:
                highest_x = panel.x

            if panel.y < lowest_y:
                lowest_y = panel.y
            if panel.y > highest_y:
                highest_y = panel.y
        ...
        full_canvas = [
            [
                self.get_panel(pos_x, pos_y) for pos_x in range(lowest_x, highest_x)
            ] for pos_y in range(lowest_y, highest_y)
        ]

        return (
            "\n".join(
                ["".join([str(panel) for panel in row]) for row in full_canvas]
            )
        )
